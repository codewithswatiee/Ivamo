...
export default function PageLoader() {
  ...
  useEffect(() => {
    ...
    // Give React and Next.js a short moment to render the DOM elements
    const timer = setTimeout(() => {
      if (!isMounted) return

      // Find all image tags and elements with inline background images
      // Ignore images with loading="lazy" to avoid indefinite loading
      const imgElements = Array.from(document.querySelectorAll('img:not([loading="lazy"])'))
        ...
        allResources.push(
          new Promise(resolve => {
            if (img.complete) {
              ...
              img.addEventListener('error', resolve, { once: true }) // proceed on error too
            }
          })
        )
      })
      ...
      if (allResources.length === 0) {
        setProgress(100)
        setTimeout(() => { if (isMounted) setVisible(false) }, 500)
        return
      }
      ...
      // Fallback: don't let it load forever
      const fallback = setTimeout(() => {
        if (isMounted) {
          setProgress(100)
          setTimeout(() => setVisible(false), 500)
        }
      }, 5000) // 5s timeout max for loading screen
